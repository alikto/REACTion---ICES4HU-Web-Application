...
import "./InstructorSurveyCreation.css";
import { useLocation } from "react-router-dom";



function InstructorSurveyCreation() {

    const location = useLocation();
    const instructorId = location.state.userID;
    const surveyType = "InstructorSurvey";
    const [selectCourseInst, setSelectCourseInst] = useState('');
    const [inputFieldsInst, setInputFieldsInst] = useState([
    ]);
    ...
    const deleteItem = (item) => {
        let newProducts = inputFieldsInst.filter(
          (record) => record.description !== item
        );
        setInputFieldsInst(newProducts);
      };
    ...
    const addNewItem = () => {
        const copyCart = [...inputFieldsInst];
        copyCart.push({"description": input});
        setInputFieldsInst(copyCart);
        setInput("");
      };
    ...
    const handleSubmit = async (event) => {
        ...
        if (inputFieldsInst.length===0) {
          alert("No questions were added!");
        }
        else if(selectCourseInst==='') {
          alert("No course was selected!");
        }
        else {
          const body = {  "instructorId" : instructorId, "courseId": selectCourseInst, "surveyType": surveyType, "questionDTOS": inputFieldsInst };
    ...
    const data = [
        { id: "1", question: "Did you like the course" },
        { id: "2", question: "Online or face to face"},
        { id: "3", question: "anything else true or false"},
      ]
    ...
    return(
        <div>
            <div className="createSurveyHeader">
                <p>Create Survey About Yourself</p>
            </div>
            <div className='input_button_dropdown_row'>
                ...
                <div className='button-get-common-questions'>
                    <button className="btn btn-danger mt-3" onClick={() => setInputFieldsInst(commonQuestions)}>Get Common Questions</button>
                </div>
